...
def pdbedit_user(module, name):
	...
	if not out: return dict(state = "absent", password = None)
	entries = dict(parse(line) for line in pdbedit.stdout.splitlines())
	return dict(state = "present", password = entries["NT hash"])
...
def adjust(module, name, expected, actual):
	if expected["state"] == "present" and actual["state"] == "absent": return pdbedit_create(module, name, expected["password"])
	if expected["state"] == "absent" and actual["state"] == "present": return pdbedit_delete(module, name)
...
def process(module, name, state, password, check):
	...
	actual = pdbedit_user(module, name)
